...
@Component({
  ...
  styleUrls: ['./task-documents.component.less'],
  ...
  ngOnInit(): void {
    ...
    this.listenerFn = this.renderer.listen('document', 'click', (event) => {
      ...
      if (event.target.hasAttribute("data-info")) {
       // console.log("task-management/view/" + event.target.getAttribute("data-info"));
       this.router.navigate(["task-management/view/" + event.target.getAttribute("data-info")]);
      }
  ...
  getTableData(){
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        that.documentApiService.getAllTaskDocuments
          (
      ...
      {title:"document",data:'document_path'},
      ...
      {title:"date",data:'created_at'},
       ...
       // htm += '<a  role="button"  data-toggle="tooltip" title="CheckIn" ><i data-checkin='+row.document_id+' class="fa fa-sign-in-alt tx-primary mg-r-6"></i></a>';
        ...
        htm += '<a   role="button"  data-toggle="tooltip" title="Information" ><i data-info='+row.task_id+' class="fa fa-info-circle tx-danger mg-r-6"></i></a>';
