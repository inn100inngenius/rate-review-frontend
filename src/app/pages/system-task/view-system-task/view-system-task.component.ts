...
@Component({
  selector: 'app-view-system-task',
  templateUrl: './view-system-task.component.html',
  styleUrls: ['./view-system-task.component.less']
})

export class ViewSystemTaskComponent implements OnInit {
  ...
  private SpinnerService: NgxSpinnerService,private systemTaskApiService:SystemTaskApiService) { }
  ...
  getSystemTaskById() {
    ...
    var _this = this;
    this.taskApiService.getSystemTaskById(params).subscribe(
      async data => {
        _this.SpinnerService.hide();
        //alert();
        console.log(data);
        let taskDetails = data[0];
        this.jsonData = JSON.parse(taskDetails.form_data);
      ...
      let totalPercentage =0;
        ...
        {
          this.hasForm = true;
          this.hasFormValue = u.form_values;
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // }
    onSubmit(event){
      let uid = this.localStorageService.getValue().userId;
        this.SpinnerService.show();
        console.log(event);
        ...
        var formData = {
          ...
          task_id:this.id,
          form_value :event.data
        }
        this.systemTaskApiService.updateFormValues(formData).subscribe(
            data => {
              console.log(data);
              this.getSystemTaskById();
     ...
     console.log(this.selectedValue);
      this.taskDetails.task_users.map(x=>{
        if(x.user_id == this.selectedValue){
          this.userTaskPercentage = (x.task_status + 1) * 33.3;
  ...
  fileUpload(){
    ...
    for(var i=0;i<this.documents.length;i++){
      input.append('task_documents[]',this.documents[i]);
    }
